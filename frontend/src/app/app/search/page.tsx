...
const SearchPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">Search Articles by Claim ID</h1>
      ...
      {articles.length > 0 ? (
        <div>
          <h2 className="text-2xl font-semibold text-gray-700 mb-4">Articles Found:</h2>
          <ul className="space-y-4">
            {articles.map(article => (
              <li key={article._id} className="bg-white shadow-md p-6 rounded-lg border border-gray-200">
                <h3 className="text-xl font-bold text-blue-600 mb-2">{article.title}</h3>
                <p className="text-gray-700"><strong>Authors:</strong> {article.authors.join(', ')}</p>
                <p className="text-gray-700"><strong>Journal:</strong> {article.journal}</p>
                <p className="text-gray-700"><strong>Year:</strong> {article.year}</p>
                <p className="text-blue-500"><strong>URL:</strong> <a href={article.url} target="_blank" rel="noopener noreferrer" className="underline">{article.url}</a></p>
                <pre className="bg-gray-100 p-4 mt-4 text-sm rounded-lg">{article.bibtex}</pre>
              </li>
            ))}
          </ul>
        </div>
